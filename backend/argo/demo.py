...
def _demo_negotiation() -> dict[str, Any]:
    return {
        "escalation_enabled": True,
        "max_escalation_percent": 10,
...
def seed_demo_data(db: Session) -> dict[str, int]:
    ...
    for index, (handle, platform, followers, engagement, fake_pct, perf, rel, aud, overall) in enumerate(
        creators_spec, start=1
    ):
        creator = Creator(
            handle=handle,
    ...
    completed = Campaign(
        ...
        compensation_source="argo",
        compensation_locked=True,
        operation_mode="full_autonomy",
        negotiation_policy=_demo_negotiation(),
    ...
    deliverable = Deliverable(
        deal_id=completed_deal.id,
        caption="Loving this vitamin C glow #ad argo.link/demo-summer",
    ...
    db.add_all(
        [
            QACheck(
                ...
                severity="major",
                passed=False,
                findings=[{"code": "missing_disclosure", "message": "FTC disclosure missing on draft."}],
    ...
    db.add(
        LearningRun(
            ...
            database_updates={
                "strategy_prior": {"niche": DEMO_NICHE, "creator_tier": "micro"},
    ...
    # Active campaign with deals in negotiating, contracted, and QA stages
    active = Campaign(
        brand_id=brand.id,
    ...
    active_deals = [
        (creators[4], DealStatus.NEGOTIATING.value, 10_000, True, False),
    ...
    for creator, status, rate, outbound, inbound in active_deals:
        deal = Deal(
            ...
            terms_accepted=status != DealStatus.NEGOTIATING.value,
...
def _demo_campaign_ids(db: Session) -> set[str]:
    return {
        ...
        if (campaign.negotiation_policy or {}).get("demo_seed")
    }
