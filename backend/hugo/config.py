...
CAPABILITY_ENV: dict[str, str] = {
    "hermes": "HUGO_HERMES_API_KEY",
    "vision": "HUGO_NVIDIA_API_KEY",
    "stripe": "HUGO_STRIPE_SECRET_KEY and HUGO_STRIPE_WEBHOOK_SECRET",
    "gmail": "HUGO_GMAIL_ACCESS_TOKEN and HUGO_GMAIL_SENDER",
    "youtube": "HUGO_YOUTUBE_API_KEY",
    "telegram": "HUGO_TELEGRAM_BOT_TOKEN",
    "discovery": "HUGO_INFLUENCERS_CLUB_API_KEY",
}
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_file=".env", env_prefix="HUGO_", case_sensitive=False, extra="ignore"
    )

    env: str = "development"
    database_url: str = "sqlite:///./hugo.db"
    ...
    nvidia_build_model: str = "nvidia/llama-3.3-nemotron-super-49b-v1"
    ...
    gmail_sender: str = ""
    email_transport: Literal["gmail_api", "browser"] = "gmail_api"
    browser_email_provider: Literal["gmail", "outlook"] = "gmail"
    browser_email_sender: str = ""
    nvidia_api_key: str = ""
    ...
    def capability_configured(self, capability: str) -> bool:
        if capability == "gmail":
            if self.email_transport == "browser":
                return bool(self.browser_email_sender)
            renewable = all(
    ...
    def validate_runtime(self) -> None:
        ...
        if self.api_token in ("", "change-me"):
            problems.append("HUGO_API_TOKEN must be set to a non-default value")
        if self.agent_token in ("", "change-agent-token"):
            problems.append("HUGO_AGENT_TOKEN must be set to a non-default value")
        for capability in REQUIRED_CAPABILITIES:
            if not self.capability_configured(capability):
                if capability == "gmail" and self.email_transport == "browser":
                    problems.append("HUGO_BROWSER_EMAIL_SENDER is required for browser email")
                else:
                    problems.append(f"{CAPABILITY_ENV[capability]} is required")
        if not self.require_nemoclaw:
            problems.append("HUGO_REQUIRE_NEMOCLAW must remain true")
