...
logger = logging.getLogger("hugo.providers")
    ...
    def outreach(self, context: dict[str, Any]) -> str:
        prompt = {
            ...
            "content": (
                ...
                "tracking requirements. State that the compensation is fixed and ask the creator "
                "to reply ACCEPT or DECLINE. Do not include or mention a web portal. Context: "
                + json.dumps(context)
            ),
        }
        ...
        account = client.v1.accounts.create(
            {
                ...
                "metadata": {"hugo_creator_id": creator_id},
            }
        )
...
class MailProvider:
    ...
    def send(
        ...
        *,
        ...
        message["X-Hugo-Idempotency-Key"] = idempotency_key
...
class DemoHermesProvider(HermesProvider):
    ...
    def strategy(self, context: dict[str, Any]) -> HermesStrategy:
        if self.settings.nvidia_build_api_key:
            return HermesProvider.strategy(self, context)
    ...
    def playbook(self, platform: str) -> dict[str, Any]:
        if self.settings.nvidia_build_api_key:
            return HermesProvider.playbook(self, platform)
        return {
            "platform": platform,
    ...
    def learning(self, dossier: dict[str, Any]) -> HermesLearning:
        if self.settings.nvidia_build_api_key:
            return HermesProvider.learning(self, dossier)
    ...
    def outreach(self, context: dict[str, Any]) -> str:
        if self.settings.nvidia_build_api_key:
            return HermesProvider.outreach(self, context)
    ...
    def _chat(self, messages: list[dict[str, str]]) -> str:
        if self.settings.nvidia_build_api_key:
            headers = {"Authorization": f"Bearer {self.settings.nvidia_build_api_key}"}
            response = httpx.post(
                "https://integrate.api.nvidia.com/v1/chat/completions",
                headers=headers,
                json={
                    "model": self.settings.nvidia_build_model,
                    "messages": messages,
                },
                timeout=120,
            )
