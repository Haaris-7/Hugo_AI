...
ENV_PATH = Path(os.environ.get("HUGO_ENV_FILE", ".env"))
...
WIZARD_FIELDS: list[Field] = [
    Field(
        "HUGO_HERMES_BASE_URL", "NemoClaw Hermes base URL", "http://host.docker.internal:8642/v1"
    ),
    ...
    Field("HUGO_STRIPE_SECRET_KEY", "Stripe secret key", secret=True),
    Field("HUGO_STRIPE_WEBHOOK_SECRET", "Stripe webhook secret", secret=True),
    Field("HUGO_GMAIL_ACCESS_TOKEN", "Gmail OAuth access token", secret=True),
    Field("HUGO_GMAIL_CLIENT_ID", "Google OAuth client ID", secret=True),
    Field("HUGO_GMAIL_CLIENT_SECRET", "Google OAuth client secret", secret=True),
    Field("HUGO_GMAIL_REFRESH_TOKEN", "Google OAuth refresh token", secret=True),
    Field("HUGO_GMAIL_SENDER", "Gmail sender address"),
    Field("HUGO_EMAIL_TRANSPORT", "Email delivery mode", "gmail_api"),
    Field("HUGO_BROWSER_EMAIL_PROVIDER", "Browser email provider", "gmail"),
    Field("HUGO_BROWSER_EMAIL_SENDER", "Browser sender email address"),
    Field("HUGO_DISCOVERY_MODE", "Creator discovery method", "hermes_agents"),
    Field("HUGO_INFLUENCERS_CLUB_API_KEY", "Influencers.club API key", secret=True),
    Field("HUGO_YOUTUBE_API_KEY", "YouTube Data API key", secret=True),
    Field("HUGO_TELEGRAM_BOT_TOKEN", "Telegram bot token", secret=True),
]
...
def summary() -> dict:
    ...
    required_env = dict(CAPABILITY_ENV)
    if settings.email_transport == "browser":
        required_env["gmail"] = "HUGO_BROWSER_EMAIL_SENDER and a Hermes-connected browser"
    return {
...
def apply_updates(updates: dict[str, str]) -> dict:
    ...
    previous = read_env()
    prev_demo = previous.get("HUGO_DEMO_MODE", "false").lower() in ("true", "1", "yes")
    write_env(filtered)
    get_settings.cache_clear()
    new_demo = read_env().get("HUGO_DEMO_MODE", "false").lower() in ("true", "1", "yes")
