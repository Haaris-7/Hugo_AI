---
name: hugo-creator-discovery
description: Provision and operate creator discovery for a live Hugo campaign.
---

# Creator discovery

Call `hugo_request_discovery` with the campaign ID. The backend applies policy
...
1. Check service-spend credits before discovery. If the provider requires a refill,
   use `hugo_request_service_spend` within the campaign's approved service-spend policy.
...
After discovery, mark the Hermes task complete with `hugo_complete_task` including
the deal IDs in the result, or fail it with `hugo_fail_task`.
